...
function modalHtml(lang, item, fields) {
  return `
    <div class="row g-3">
      ...
      ` : ""}
      ${fields.showInForm("requires_imei") ? `
        <div class="col-md-6">
...
function readForm(modalEl) {
  ...
  return {
    ...
    requires_imei: byName("requires_imei")?.checked ? 1 : 0,
        ...
        <table class="table table-sm table-hover align-middle mb-0">
          <thead>
            <tr>
              ...
              ${fields.showInList("requires_imei") ? `<th style="width:150px">${esc(text(lang, "imei"))}</th>` : ""}
          ...
          <tbody>
            ${items.map(item => `
              <tr>
                ...
                ${fields.showInList("requires_imei") ? `<td>${ynBadge(item.requires_imei, labels)}</td>` : ""}
                ...
                <td>
                  <div class="d-flex gap-2 flex-wrap">
                    ...
                    ${fields.isEnabled("is_active") ? `<button class="btn btn-sm btn-outline-secondary" data-toggle-product-type="${item.id}" data-next="${item.is_active ? 0 : 1}">${item.is_active ? esc(text(lang, "inactive")) : esc(text(lang, "active"))}</button>` : ""}
                  </div>
                </td>
              </tr>
            `).join("")}
      ...
      ${items.map(item => `
        <div class="card mb-2 shadow-sm">
          <div class="card-body p-3">
            ...
            ${fields.showInCard("requires_imei") ? `<div class="small text-muted mt-2">${esc(text(lang, "imei"))}: ${item.requires_imei ? esc(text(lang, "yes")) : esc(text(lang, "no"))}</div>` : ""}
...
export async function render(ctx) {
  ...
  try {
    [resp, fields] = await Promise.all([
      api("/product-types"),
      loadEntityFieldAccess(api, "product_types")
    ]);
  } catch (e) {
  ...
  document.getElementById("settings_product_types_create").addEventListener("click", () => openEntityModal(ctx, null, fields));
